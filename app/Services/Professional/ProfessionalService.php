...
class ProfessionalService {
    ...
    public function getProfessionalDashboardProfile(int $userId)
    {
        ...
        $reviewsCount = Review::whereHas('order.service', function ($query) use ($professional) {
            $query->where('professional_id', $professional->id);
        })->count();

        $recentReviews = Review::with(['client', 'order.service'])
    ...
    protected function buildRequestsOverTime(Collection $requests): array
    {
        ...
        $requestData = $weeks->map(function ($week) use ($requests) {
            return $requests->filter(function ($request) use ($week) {
                return $request->created_at
                    && Carbon::parse($request->created_at)->between($week['start'], $week['end']);
            })->count();
        })->values()->all();

        $completedData = $weeks->map(function ($week) use ($requests) {
            return $requests->filter(function ($request) use ($week) {
                return $request->status === 'Terminer'
                    && $request->created_at
                    && Carbon::parse($request->created_at)->between($week['start'], $week['end']);
            })->count();
        })->values()->all();
    ...
    protected function buildTopServices(Collection $services): array
    {
        $topServices = $services
            ->map(function ($service) {
                $requestsCount = collect($service->requests ?? [])
                    ->where('is_canceled', false)
                    ->count();
    ...
    protected function buildRatingSummary(Collection $reviews): array
    {
        ...
        $ratings = collect([5, 4, 3, 2, 1])->map(function ($score) use ($reviews, $totalReviews) {
            $count = $reviews->filter(function ($review) use ($score) {
                return (int) round($review->rating) === $score;
            })->count();
        ...
        return [
            ...
            'reviewsLabel' => 'Based on ' . $totalReviews . ' reviews',
            'ratings' => $ratings,
        ];
    }
}
